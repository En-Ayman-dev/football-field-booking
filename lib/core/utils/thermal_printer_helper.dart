...
class ThermalPrinterHelper {
  ...
  static Future<void> printReport({
    ...
    double totalRevenue = reports.fold(0, (sum, item) => sum + item.totalAmount);
  ...
  static Future<Uint8List> _generateSingleBookingImage({
    ...
    // تقدير الطول: هيدر + 6 أسطر بيانات + فوتر
    double height = 550.0; 
    canvas.drawRect(Rect.fromLTWH(0, 0, receiptWidth.toDouble(), height), paint);
    ...
    // العنوان
    yOffset = _drawText(canvas, "Arena Manager ", yOffset, fontSize: titleFontSize, isBold: true, isCentered: true);
    yOffset = _drawText(canvas, " إيصال حجز ملعب نادي مدينة ذمار", yOffset, fontSize: 22, isBold: true, isCentered: true);
    ...
    // المبلغ والحالة
    yOffset = _drawText(canvas, "المبلغ المطلوب", yOffset, isCentered: true, fontSize: 18);
    yOffset = _drawText(canvas, "${amount.toStringAsFixed(2)} ريال", yOffset, isCentered: true, isBold: true, fontSize: 28);
    
    String statusText = isPaid ? "تم السداد" : "غير مسدد";
    yOffset = _drawText(canvas, "الحالة: $statusText", yOffset, isCentered: true, fontSize: 20);
    
    yOffset = _drawDivider(canvas, yOffset);
    _drawText(canvas, "نتمنى لكم وقتاً ممتعاً", yOffset, isCentered: true, fontSize: 16);
  ...
  static Future<Uint8List> _generateReportImage(
    ...
    // الهيدر
    yOffset = _drawText(canvas, "Arena Manager", yOffset, fontSize: titleFontSize, isBold: true, isCentered: true);
    yOffset = _drawText(canvas, "تقرير مالي نادي مدينة ذمار ", yOffset, fontSize: 22, isBold: true, isCentered: true);
    yOffset = _drawText(canvas, "$start :إلى $end", yOffset, isCentered: true, fontSize: 16);
    yOffset = _drawDivider(canvas, yOffset);

    // الجدول
    for (var report in reports) {
      yOffset = _drawKeyValue(canvas, report.formattedDate, "", yOffset, isBoldKey: true);
      yOffset = _drawKeyValue(canvas, "الدخل:", report.totalAmount.toStringAsFixed(2), yOffset);
    ...
    yOffset += 10;
    yOffset = _drawDivider(canvas, yOffset);
    yOffset = _drawText(canvas, "الإجمالي الكلي", yOffset, isCentered: true, fontSize: 18);
    yOffset = _drawText(canvas, "${totalRevenue.toStringAsFixed(2)} ريال", yOffset, isCentered: true, isBold: true, fontSize: 28);
  ...
  static double _drawText(Canvas canvas, String text, double y, {bool isBold = false, bool isCentered = false, double fontSize = baseFontSize}) {
  ...
  static double _drawKeyValue(Canvas canvas, String key, String value, double y, {bool isBoldKey = false}) {
    final textStyle = TextStyle(
      color: Colors.black,
    ...
    final valuePainter = TextPainter(
      ...
      textDirection: pw.TextDirection.ltr, // الأرقام والإنجليزي LTR
    );
  ...
  static double _drawDivider(Canvas canvas, double y) {
    ...
    canvas.drawLine(Offset(0, y + 10), Offset(receiptWidth.toDouble(), y + 10), paint);
    return y + 20;
  }
  ...
  static Future<Uint8List> _toMonochromePng(ui.Image img) async {
    ...
    final ui.Image highRes = await recorderUp
        .endRecording()
        .toImage(img.width * scale, img.height * scale);

    // 2) استخراج RGBA
    final byteData =
        await highRes.toByteData(format: ui.ImageByteFormat.rawRgba);
    ...
    final ui.Image finalImg =
        await recorderDown.endRecording().toImage(img.width, img.height);
